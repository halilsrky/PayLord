 ...
 */
void dwt_profiler_print_results(void (*uart_send_func)(uint8_t* data, uint16_t size))
{
    static char buffer[1024];
    
    if (!profiler_initialized || !uart_send_func) {
        return;
    }
    ...
    if (len > 0 && len < sizeof(buffer)) {
        uart_send_func((uint8_t*)buffer, len);
    }
}
 ...
 */
void dwt_profiler_print_compact(void (*uart_send_func)(uint8_t* data, uint16_t size))
{
